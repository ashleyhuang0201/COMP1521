...
#DIS IS THE PART ASSLEY WROTE
    move $s0, $v0 #moves v0 value into s0 which stands for n 
    
    li $s1, 1 #initialise int i = 1
    li $s2, 1 #initialise int fac = 1
    
while:
    bgt $s1, $s0, end_while #loop
    mul $s2, $s2, $s1 #multiply s1 with s2 and store in s2
    addi $s1, $s1, 1  #i++
    j while
