...
main:
    ...
    move $a0, $v0
	 ...
	 #moves scanned value into v0
    jal fac #calls the fac function
    add $t0, $v0, $zero
...
fac:
	...
	# code for fac()
    li $s0, 1 #initalise int i = 1
    li $s1, 1 #initialise int prod = 1
    
loop:
    bgt $s0, $a0, end_loop #loop
    mul $s1, $s1, $s0 #multiply s1 with s2 and store in s2
    addi $s0, $s0, 1  #i++
    jal loop
