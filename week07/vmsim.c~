...
int physicalAddress (uint vAddr, char action)
{
	...
	int pageno = vAddr/PAGESIZE; //find page number
	uint offset = vAddr % PAGESIZE; //find offset from vAddr
	int physicalAdd = -1;
	
	if(vAddr >= (nPages * PAGESIZE)) { // if it's not valid then we return -1
		return physicalAdd;
	}
	
	if (PageTable[pageno].status.loaded == 1) { // if the page is already loaded
		if (action == 'W') { // set the modified flag if action is a write
			PageTable[pageno].status.modified = 1;
		}
		PageTable[pageno].lastAccessed = clock; //update the access time 
		physicalAdd = (PageTable[pageno].frameNo * PAGESIZE) + offset;
		//use frame number and offset to compute physicalAdd
		
	} else {
		int i;
		int frame;
		for (i = 0; i < nFrames; ++i) { // look for unused frame 
			if (MemFrames[i] == -1) {
				break; // found unused frame.
			}
		}
		if (MemFrames[i] == -1) { // if we found it, use it
			frame = i;
		} else { // if we found one then we can use it otherwise need to replace a currently loaded frame
			
			int least_time = clock;
			int j;
			int least = 0; //least recently used page 
			for (j = 0; j < nPages; j++)
			{
				if ((PageTable[j].lastAccessed <= least_time) && (PageTable[j].lastAccessed > -1))
				{
					least_time = PageTable[j].lastAccessed;
					least = j; // find the least recently used page
				}
			}
			frame = PageTable[least].frameNo; // use the frame from the old page
			if (PageTable[least].status.modified == 1) nSaves++; //increment counter if modified    

            //set PageTable entry to indicate "no longer loaded"
			PageTable[least].lastAccessed = -1;
			PageTable[least].frameNo = -1;
			PageTable[least].status.loaded = 0;
			PageTable[least].status.modified = 0; // reset everything
			nReplaces++; //increment 
		...
		if (action == 'W') { //if action is a write, set the modified flag
			PageTable[pageno].status.modified = 1;
		}
		PageTable[pageno].frameNo = frame;
		PageTable[pageno].lastAccessed = clock;
		physicalAdd = (PageTable[pageno].frameNo * PAGESIZE) + offset; //use frame number and offset to calculate physical address

	}
	return physicalAdd; //return the physical address
}
